...
function MesFactures() {
  ...
  const getStatusBorderClass = (status) => {
    ...
    if (normalized === "paid") return "border-green";
    if (normalized === "unpaid") return "border-red";
    if (normalized === "pending") return "border-yellow";
    return "border-gray";
  };

  const getStatusValueClass = (status) => {
    const normalized = normalizeInvoiceStatus(status);
    if (normalized === "paid") return "value-green";
    if (normalized === "unpaid") return "value-red";
    if (normalized === "pending") return "value-yellow";
    return "value-gray";
  };

  return (
    <div className="mes-factures-page">
      <Container className="mes-factures-container">
        <h2 className="page-title">{t("mesFactures.title")}</h2>
            ...
            <Row>
              {filteredFactures.map((facture) => (
                <Col md={3} sm={6} xs={12} className="mb-4" key={facture.id}>
                  <Card className={`invoice-card ${getStatusBorderClass(facture.status)}`}>
                    <Card.Body>
                      <h4 className="invoice-card-title">{facture.reference}</h4>
                      <div className="invoice-card-details">
                        <span className="invoice-label">{t("mesFactures.date")}</span>
                        <span className="invoice-value">{facture.date}</span>
                      </div>
                      <div className="invoice-card-details">
                        <span className="invoice-label">{t("mesFactures.price")}</span>
                        <span className={`invoice-value ${getStatusValueClass(facture.status)}`}>{facture.prix}</span>
                      </div>
                      <div className="invoice-card-details">
                        <span className="invoice-label">{t("mesFactures.status")}</span>
                        <span className={`invoice-value ${getStatusValueClass(facture.status)}`}>
                          {getTranslatedStatus(facture.status)}
                        </span>
                      </div>
                      <div className="facture-action mt-3">
                        <Button
                          size="sm"
                          className="btn-voir"
                          onClick={() => handleViewInvoice(facture.id)}
